...
class AES192(AES128):
    """AES-192 加密实现（继承 AES128，重写密钥扩展）

    采用位数组接口：明文 128 位、密钥 192 位，返回密文 128 位。
    仅实现单块加密流程（无模式/填充）。

    Args:
        rounds (int): 轮数，默认 12（有效范围 [1, 14]）
    ...
    def _key_schedule(self, key_bits: np.ndarray) -> list:
        """密钥扩展（AES-192）生成所有轮密钥

        Args:
            key_bits (np.ndarray): 192 位密钥位数组（uint8 0/1）

        Returns:
            list[np.ndarray]: 轮密钥列表，每项为 16 字节数组

        Raises:
            ValueError: 当密钥长度不是 192 位时抛出
        """
        if key_bits.size != 192:
            raise ValueError("AES192 仅支持 192 位密钥")
